...
- **2026-08-26** — ![FIX](https://img.shields.io/badge/FIX-2ea44f?style=flat-square) ✍️ **论文不再写成忏悔书**([#423](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/423);来自社区反馈)。写作 skill 教 hedge、改进循环唯一的语气操作是"软化"——还有个只会越拧越软的棘轮("FURTHER soften"),五轮下来满纸道歉。现在:每个 claim 校准到证据然后**直说**;"further research is needed" 这类通用免责**只住 Limitations**;"别提 X"就是删掉 X——绝不在正文写 "we do not address X";语气修订上了锁——**永不改变论文知道什么**,两个方向都锁。审稿人双向判校准,不许给有证据的结果索要额外 hedge。一条因果脊椎:让方法显得不可避免——结果按论证排序而不是按实验先后,对照实验紧邻它检验的 claim([#424](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/424))。其中「语气修订锁」等多条规则采自社区项目 [humanize-paper](https://github.com/SyntaxSmith/humanize-paper),感谢作者把这个病命名得如此精确。⚠️ 运行 `bash tools/smart_update.sh --apply` 拉取更新。
