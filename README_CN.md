...
- **2026-08-09** — ![NEW](https://img.shields.io/badge/NEW-red?style=flat-square) 🦆 **Copilot CLI 宿主下,`/auto-review-loop` 默认改用其原生 rubber-duck 审稿 subagent**([#360](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/360),closes [#258](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/issues/258);by [@Functionhx](https://github.com/Functionhx))。不再依赖 Codex MCP、不再固定 GPT 型号:每轮由宿主内置 subagent 审稿,helper 从宿主持久化的会话事件中提取并复验双方真实模型与跨家族关系,同家族/家族不明/证据缺失一律 fail-closed 到已知异族外部后备或 `REVIEW_UNAVAILABLE`。显式指定的外部 reviewer 照旧优先;非 Copilot 宿主仍默认 Codex。⚠️ 运行 `bash tools/smart_update.sh --apply` 拉取更新。
