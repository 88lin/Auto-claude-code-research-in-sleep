...
- **2026-08-26** — ![FIX](https://img.shields.io/badge/FIX-2ea44f?style=flat-square) ✍️ **Papers stop reading like confession letters** ([#423](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/423); driven by community reports). The writing skills taught hedging and the improvement loop's only tone operation was *soften* — with a round-2 ratchet ("FURTHER soften any remaining overclaims") that turned five rounds into an apology. Now: calibrate each claim to its evidence and **state it directly**; generic caveats ("further research is needed", "may not generalize") live in the Limitations section **only**; "don't mention X" means omit X — never "we do not address X" in the manuscript; and tone edits are locked — they never change what the paper knows, in either direction. Reviewers now judge calibration both ways and may not request extra hedges around a supported result. One causal spine: make the method feel inevitable — with results ordered by argument, not lab-notebook chronology, and controls sitting next to the claims they test ([#424](https://github.com/wanshuiyin/Auto-claude-code-research-in-sleep/pull/424)). The claims-lock and several of these rules are adopted from the community's [humanize-paper](https://github.com/SyntaxSmith/humanize-paper) — thanks to its author for naming the disease precisely. ⚠️ Run `bash tools/smart_update.sh --apply` to pull.
